...
bool doesPairContainAnyOverlap(int e1s, int e1e, int e2s, int e2e){
    if(doesPairContainTotalOverlap(e1s, e1e, e2s, e2e) ||
        e1s <= e2s && e1e >= e2s || e2s <= e1s && e2e >= e1s){
        return true;
    }
    return false;
}
...
int main(){
    ...
    while(std::getline(allPairs, line)){
        ...
        if (doesPairContainAnyOverlap(elf1Start, elf1End, elf2Start, elf2End)){
            allTotalOverlap++;
        }
    }
    cout << "Total pairs that have any overlap : " << allTotalOverlap << endl;
